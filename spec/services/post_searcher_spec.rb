...
RSpec.describe PostSearcher do
  it "finds all when no arguments given" do
    create_list(:post, 4)
    results = PostSearcher.new.search(params: {})
  ...
  it "filters by continuity" do
    ...
    create(:post)
    results = PostSearcher.new.search(params: { board_id: post.board_id })
  ...
  it "filters by setting" do
    ...
    create(:post)
    results = PostSearcher.new.search(params: { setting_id: setting.id })
  ...
  it "filters by subject" do
    ...
    results = PostSearcher.new.search(params: { subject: 'stars' })
  ...
  it "filters by authors" do
    ...
    results = PostSearcher.new.search(params: { author_id: [filtered_post.user_id] })
  ...
  it "filters by multiple authors" do
    ...
    results = PostSearcher.new.search(params: { author_id: [author1.id, author2.id] })
  ...
  it "filters by characters" do
    ...
    results = PostSearcher.new.search(params: { commit: true, character_id: reply.character_id })
  ...
  it "filters by completed" do
    ...
    results = PostSearcher.new.search(params: { completed: true })
    expect(results).to match_array(post)
  end
