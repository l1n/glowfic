...
RSpec.describe ReplySearcher do
  it "finds all when no arguments given" do
    create_list(:reply, 4)
    results = ReplySearcher.new(templates: [], users: []).search(params: {})
  ...
  it "filters by author" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { author_id: filtered_reply.user_id })
  ...
  it "filters by icon" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { icon_id: reply.icon_id })
  ...
  it "filters by character" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { character_id: reply.character_id })
    expect(results).to match_array([reply])
  end
  ...
  it "filters by string" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { subj_content: 'seagull' })
  ...
  it "filters by exact match" do
    ...
    create(:reply, content: 'nope')
    results = ReplySearcher.new(templates: [], users: []).search(params: { subj_content: '"Forks High"' })
    expect(results).to match_array([reply])
  end
  ...
  it "filters by post" do
    replies = create_list(:reply, 4)
    filtered_reply = replies.last
    results = ReplySearcher.new(templates: [], users: []).search(params: {}, post: filtered_reply.post)
    expect(results).to match_array([filtered_reply])
  end
  ...
  it "filters by continuity" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { board_id: continuity_post.board_id })
    expect(results).to match_array([filtered_reply])
  end
  ...
  it "filters by template" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { template_id: character.template_id })
    expect(results).to match_array([reply])
  end
  ...
  it "sorts by created desc" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { sort: 'created_new' })
  ...
  it "sorts by created asc" do
    ...
    results = ReplySearcher.new(templates: [], users: []).search(params: { sort: 'created_old' })
